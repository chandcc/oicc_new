...
(function($) {
	...
	function setSelectControl(oSelect, iStart, iLength, iIndex) {
		...
		for (var i = 0; i < iLength; i++) {
			var formartValue=(parseInt(iStart) + i)+"";
			if(formartValue.length == 1) formartValue="0"+formartValue;
			if ((parseInt(iStart) + i) == iIndex)
     			oSelect.append("<option selected='selected' value='"
						+ formartValue + "'>"
						+ formartValue + "</option>");
			
			else
				oSelect.append("<option value='" + formartValue
						+ "'>" + formartValue + "</option>");
		}
	}
	...
	$.fn.DateSelector = function(options) {
		...
		var dt = new Date(), iMonth = parseInt(this.monthValueId.attr("data")
				|| this._options.defMonth), iDay = parseInt(this.dayValueId
				.attr("data")
				|| this._options.defDay), iMinYear = parseInt(this._options.minYear), iMaxYear = parseInt(this._options.maxYear);

		this.Year = parseInt(this.yearValueId.attr("data")
				|| this._options.defYear)
				|| dt.getFullYear();
		...
		// 初始化控件
		// 设置年
		setSelectControl(this.yearValueId, this.minYear, this.maxYear
				- this.minYear + 1, this.Year);
		// 设置月
		setSelectControl(this.monthValueId, 1, 12, this.Month);
		// 设置日
		var daysInMonth = new Date(this.Year, this.Month, 0).getDate(); // 获取指定年月的当月天数[new
		// Date(year,
		// month,
		// 0).getDate()]
		...
		$("#idYear").val(options.defYear);
		$("#idMonth").val(options.defMonth);
		$("#idDay").val(options.defDay);
	}
})(jQuery);