...
public interface TMemberMapper extends Mapper<TMember> {
    ...
    @Select({"<script>select *  from t_member where ${tableName} =1 <when test='mobile!=null'>and  (mobile like CONCAT('%',#{mobile},'%') " +
            "or id like CONCAT('%',#{mobile},'%'))</when> order by ${time}  desc </script>"})
    ...
    @Select({"<script>select * from t_member  where ${tableName}  =1 and  id not in (select t_user_id from t_production where type= #{type} )  " +
            "<when test='mobile!=null'>and (mobile like CONCAT('%',#{mobile},'%') or id like CONCAT('%',#{mobile},'%') ) </when>" +
            "order by ${time}  desc</script>"})
    List<Map<String, Object>> paginate3(@Param("tableName") String tableName, @Param("time") String time, @Param("mobile") String mobile);

    @Select({"<script>select t.*  from t_member t,t_production p where t.#{tableName} =1 and p.type=#{type}  and t.id=p.t_user_id <when test='mobile!=upload'>and  p.is_upload=#{upload}" +
            " </when><when test='mobile!=mobile'> and (t.mobile like CONCAT('%',#{mobile},'%') or t.id like CONCAT('%',#{mobile},'%') ) </when >order by ${time}  desc</script>"})
    List<Map<String, Object>> paginate4(@Param("tableName") String tableName, @Param("time") String time, @Param("mobile") String mobile, @Param("upload") int upload);
}